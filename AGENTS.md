...
- Live T3 on this machine (`lab`) is systemd `t3code.service`, served from an immutable release directory under `~/.local/lib/t3` that an ordered systemd drop-in selects. Restarting it without staging a new release leaves the old code running. When the user wants worktree server changes on that live T3, use `.agents/skills/deploy-t3-lab/SKILL.md`. Isolated testing still uses `vp run dev` and the worktree `.t3`.
