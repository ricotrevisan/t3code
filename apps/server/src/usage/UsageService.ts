...
export const make = Effect.gen(function* () {
  ...
  const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(function* (
    ...
    return [
      { scanKind: "claude" as const, dir: claudeDir },
      { scanKind: "codex" as const, dir: path.join(codexLayout.sharedHomePath, "sessions") },
      {
        scanKind: "grok" as const,
        dir: path.join(grokHome, "sessions"),
        fileName: "updates.jsonl",
      },
      {
        scanKind: "primeAgent" as const,
        dir: path.join(config.baseDir, "prime-agent", "sessions"),
      },
    ];
  });
  ...
  const collectDirs = Effect.fn("UsageService.collectDirs")(function* (
    ...
    for (const { scanKind, dir, fileName } of dirs) {
  ...
  const scanSummary = Effect.fn("UsageService.scanSummary")(function* (
    ...
    for (const { scanKind, dir, volumeId, files } of scannedDirs) {
      ...
      if (files === null) {
        // Prime Agent's directory is environment-local and absent until the
        // first session; that is not a failed native home.
        if (scanKind === "primeAgent") continue;
        sources.push({
          fingerprint: {
            hostId,
            provider: scanKind,
      ...
      if (scanKind === "primeAgent") {
        ...
        for (const [provider, group] of byProvider) {
          sources.push({
            fingerprint: { hostId, provider, harness, resolvedHomePath: dir, volumeId },
            status: "ok",
