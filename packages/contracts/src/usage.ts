 ...
 *
 * Each environment scans on-disk session transcripts rather than relying on
 * T3 Code's own orchestration projections, so usage stays complete even for
 * turns that were never driven through T3 Code. Native CLIs are read from
 * their homes (`~/.claude/projects/**\/*.jsonl`, `~/.codex/sessions/**\/*.jsonl`,
 * `~/.grok/sessions/**\/updates.jsonl`). Prime Agent sessions live under the
 * environment's own base dir (`prime-agent/sessions/**\/*.jsonl`) and are
 * attributed to the upstream provider while preserving Prime Agent as the
 * harness.
 *
 * Environments return pre-aggregated
 * `(day, hourStart?, provider, harness?, model)` buckets. Raw transcript
 ...
 *
 * v6 adds `opencode`/`unknown` and an optional `harness` on buckets and
 * source fingerprints. v5 Grok buckets and v4 Claude/Codex buckets remain
 * valid, and a missing `harness` is native, so mixed-version environments
 * keep those totals instead of treating every older server as stale.
 */
 ...
 *
 * Omitted on summaries from servers that predate v6; clients treat that as
 * `native`.
 */
